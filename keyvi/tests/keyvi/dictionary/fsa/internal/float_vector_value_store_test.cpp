...
BOOST_AUTO_TEST_CASE(minimization) {
  FloatVectorValueStore<5> float_store(
      keyvi::util::parameters_t{{TEMPORARY_PATH_KEY, "/tmp"}, {"memory_limit_mb", "10"}});
...
BOOST_AUTO_TEST_CASE(persistence) {
  ...
  std::array<float, 5> v({1.2, 1.3, 1.4, 1.5, 1.6});
  std::array<float, 5> w({1.2, 1.888, 1.44235, 2.5, 1.36});
