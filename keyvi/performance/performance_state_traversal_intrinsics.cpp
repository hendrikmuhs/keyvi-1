...
clock_type::duration sse42_2(std::vector<std::vector<unsigned char>>& test_states) {
  auto const start = clock_type::now();
  ...
  for (auto it : test_states) {
    ...
    for (int offset = 0; offset < 16; ++offset) {
      ...
      uint16_t mask_int = _mm_extract_epi16(mask, 0);
...
int main() {
  ...
  auto duration_sse42_2 = sse42_2(test_states);
  std::cout << "SSE4.2(2) implementation: " << duration_sse42_2.count() / rounds << "ns" << std::endl;
  auto duration_generic2 = generic(test_states);
  std::cout << "Generic implementation: " << duration_generic2.count() / rounds << "ns" << std::endl;
