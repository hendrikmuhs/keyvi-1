...
inline void EncodeFloatVector(std::function<void(compression::buffer_t*, const char*, size_t)> compress,
                              std::array<uint32_t, N>* helper_buffer, compression::buffer_t* compression_buffer,
  ...
  // todo: do not copy if little endian already
...
template <size_t N>
inline std::string EncodeFloatVector(std::array<float, N> value) {
  std::array<uint32_t, N> helper_buffer;
