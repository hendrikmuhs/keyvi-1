...
class FloatVectorValueStoreBase {
 public:
  using value_t = std::array<float, N>;
...
template <std::size_t N>
class FloatVectorValueStoreMinimizationBase : public FloatVectorValueStoreBase<N> {
 public:
  explicit FloatVectorValueStoreMinimizationBase(
...
template <std::size_t N>
class FloatVectorValueStore final : public FloatVectorValueStoreMinimizationBase<N> {
 public:
  using typename FloatVectorValueStoreBase<N>::value_t;

 private:
  using FloatVectorValueStoreBase<N>::number_of_values_;
  using FloatVectorValueStoreBase<N>::number_of_unique_values_;
  using FloatVectorValueStoreBase<N>::values_buffer_size_;
  using FloatVectorValueStoreMinimizationBase<N>::hash_;
  using FloatVectorValueStoreMinimizationBase<N>::values_extern_;
 ...
 private:
  ...
  bool minimize_ = true;
  std::array<uint32_t, N> float_mapped_to_uint32_buffer_;
...
class FloatVectorValueStoreMergeBase {
 public:
  using value_t = std::array<float, 1>;
...
class FloatVectorValueStoreReader final : public IValueStoreReader {
 public:
  ...
  void CheckCompatibility(const IValueStoreReader& other) {
...
struct ValueStoreComponents<value_store_t::FLOAT_VECTOR> {
  template <std::size_t N>
  using value_store_writer_t = FloatVectorValueStore<N>;
